                ...
                <tbody>
                    ...
                    @foreach ($orders as $order)
                        <tr>
                            ...
                            <td><span class="inv-email"> Nº {{$order->id }}</span></td>
                            <td><p class="align-self-center mb-0 user-name"> {{ $order->customer->name }} </p></td>
                            <td><span class="inv-email"> {{ substr($order->obs, 0, 20)  }}...</span></td>
                            <td><span class="inv-amount">R$ {{ number_format($order->total,2,',','.') }}</span></td>
                            ...
                            <td>
                                @if ($order->status == 'concluido')
                                    <form action="{{ route('order.status', $order->id) }}" method="post">
                                        ...
                                        <button type="submit" class="btn btn-success btn-sm bs-tooltip" title="Pendente">
                                            concluído
                                        </button>
                                    </form>
                                @else
                                    <form action="{{ route('order.status', $order->id) }}" method="post">
                                        ...
                                        <button type="submit" class="btn btn-outline-danger btn-sm bs-tooltip" title="Concluir">
                                            pendente
                                        </button>
                                    </form>
                                @endif
                            ...
                            <td>
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenuLink2">
                                        <a class="dropdown-item action-edit" href="{{ route('order.show', $order->id) }}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-eye"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>Visualizar</a>
                                        {{-- <a class="dropdown-item action-edit" href="{{ route('order.edit', $order->id) }}"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="feather feather-edit-3"><path d="M12 20h9"></path><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"></path></svg>Editar</a> --}}
